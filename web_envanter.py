...
def detayli_rapor_excel(df_stok, df_malzeme, df_yer, detayli_mi, fig):
    output = io.BytesIO()
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        df_stok.to_excel(writer, index=False, sheet_name='Güncel Stok')
        
        # Eğer detay istendiyse ve grafik varsa Excel'e ekle
        if detayli_mi:
            if fig is not None:
                ...
                ws = writer.sheets['Güncel Stok']
                ws.add_image(img, 'I2') # Tablonun yanına (I2 hücresine) grafiği yapıştırır
...
def detayli_rapor_word(df_stok, df_malzeme, df_yer, detayli_mi, fig):
    doc = docx.Document()
    ...
    if detayli_mi:
        # Grafiği Word'e Ekle
        if fig is not None:
            doc.add_heading('Kullanıma Hazır Mevcut Stok Grafiği', 1)
            img_stream = io.BytesIO()
            fig.savefig(img_stream, format='png', bbox_inches='tight', dpi=150)
            img_stream.seek(0)
            doc.add_picture(img_stream, width=docx.shared.Inches(6.0)) # Word sayfasına sığacak genişlik
...
with tab3:
    ...
    # 1. Önce Grafiği Çiz (İndirme Fonksiyonuna Gidilebilmesi İçin)
    ...
    # 2. İndirme Butonları (Oluşturulan figürü dahil eder)
    st.write("#### 📥 Gelişmiş Rapor Çıktısı Al")
    detay_secim = st.checkbox("İndirilecek rapora Saha Operasyon Analizlerini (En çok gidenler vb.) VE Grafikleri dahil et")
    
    col_r1, col_r2 = st.columns(2)
    with col_r1:
        st.download_button("📊 Raporu Excel İndir", data=detayli_rapor_excel(df_stok, df_analiz, df_yer, detay_secim, fig), file_name='guncel_envanter.xlsx', mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    with col_r2:
        st.download_button("📝 Raporu Word İndir", data=detayli_rapor_word(df_stok, df_analiz, df_yer, detay_secim, fig), file_name='guncel_envanter.docx', mime='application/vnd.openxmlformats-officedocument.wordprocessingml.document')

    st.markdown("---")
    
    # 3. Grafiği Ekranda Göster
    col_grafik_ekran, col_analiz_tablo = st.columns(2)
    with col_grafik_ekran:
