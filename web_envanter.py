...
# Sayfa Ayarları
st.set_page_config(page_title="Afet Eğitim ve Lojistik Merkezi", layout="wide")
...
def init_db():
    ...
    for k in ["Çadır & Barınma", "Konteyner", "Isıtıcı & Jeneratör", "Mobil Araçlar", "Hijyen & Tuvalet", "Arama Kurtarma Ekipmanı"]:
        cursor.execute('INSERT OR IGNORE INTO malzemeler (malzeme_adi, kategori, birim) VALUES (?, ?, ?)', (f"Örnek {k}", k, "Adet")) # Dummy engellemek için
    conn.commit()
    conn.close()
...
# --- ARAYÜZ BAŞLIĞI ---
st.title("🛡️ Afet Eğitim ve Lojistik Merkezi")
st.subheader("Güncel Envanter ve Saha Operasyon Portalı")
st.markdown("---")

# Sekmeler
tab1, tab2, tab3 = st.tabs(["📦 1. Malzeme Tanımlama", "🚚 2. Depo İşlemleri (Sevk/Zayi)", "📊 3. Analiz ve Raporlar"])
...
with tab1:
    ...
    with col1:
        ...
        st.write("### Malzeme Sil")
        malzemeler_df = get_data("SELECT id, malzeme_adi FROM malzemeler ORDER BY malzeme_adi")
        silinecek = st.selectbox("Sistemden tamamen silinecek malzemeyi seçin:", malzemeler_df['malzeme_adi'].tolist() if not malzemeler_df.empty else ["Kayıt Yok"])
        if st.button("Seçili Malzemeyi Sil", type="primary"):
            if silinecek != "Kayıt Yok":
                run_query("DELETE FROM malzemeler WHERE malzeme_adi = ?", (silinecek,))
                run_query("DELETE FROM hareketler WHERE malzeme_adi = ?", (silinecek,))
                st.success(f"{silinecek} tamamen silindi.")
                st.rerun()
...
with tab2:
    ...
    with col_islem:
        ...
        with st.form("islem_form"):
            ...
            yer = st.text_input("İlgili Yer / Etkinlik (Örn: Beypazarı)")
    ...
    with col_gecmis:
        st.write("### Son Stok Hareketleri (Defter)")
        df_hareket = get_data("SELECT id as 'İşlem No', tarih as Tarih, malzeme_adi as Malzeme, islem_tipi as 'İşlem Tipi', miktar as Miktar, yer as 'Görev Yeri' FROM hareketler ORDER BY id DESC")
...
with tab3:
    ...
    # Veriyi çek ve pandas ile hesapla
    ...
    # Sütun isimlerini düzenle ve sırala
