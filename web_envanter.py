...
# --- KURUMSAL BAŞLIK VE LOGOLAR (TRY-EXCEPT KALKANI İLE) ---
col_logo1, col_baslik, col_logo2 = st.columns([1, 4, 1])

with col_logo1:
    try:
        # Uzantılar tam olarak senin belirttiğin gibi büyük harfli
        st.image("abb.JPG", use_column_width=True)
    except Exception as e:
        st.warning("ABB Logosu formata uygun değil veya eksik.")
...
with col_logo2:
    try:
        st.image("db.JPG", use_column_width=True)
    except Exception as e:
        st.warning("Daire Logosu formata uygun değil veya eksik.")
